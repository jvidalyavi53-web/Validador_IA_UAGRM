...
from backend.database.session import init_db, get_db, engine
...
def get_user_cluster(username: str, provider: str = "groq"):
    ...
    if key not in active_clusters:
        logger.info(
            f"Creando partición RAM exclusiva para: {username} (provider={provider})"
        )
        new_vdb = VectorDB()
        new_engine = RAGEngine(vdb_instance=new_vdb, provider=provider)
        active_clusters[key] = {"vdb": new_vdb, "engine": new_engine}
    return active_clusters[key]
...
async def upload_document(
    ...
    cluster = get_user_cluster(current_user.username)
    ...
    try:
        ...
        logger.info(f"Vectorizando: {file.filename}")
        ...
        logger.info(f"Vectorización completada: {file.filename}")
    ...
    return {
        "message": "Documento blindado e ingestado en Base Vectorial.",
        "status": "success",
...
async def ask_knowledge_base(
    ...
    cluster = get_user_cluster(current_user.username, provider=payload.provider)
    engine_obj = cluster["engine"]
    engine_obj.set_provider(payload.provider)

    try:
        respuesta = await asyncio.wait_for(
            asyncio.to_thread(engine_obj.ask, payload.query),
...
async def clear_vector_ram(
    ...
    try:
        for (username, provider), cluster in list(active_clusters.items()):
            cluster["vdb"].reset()
            cluster["engine"] = RAGEngine(
                vdb_instance=cluster["vdb"], provider=provider
            )
